#!/bin/bash
# ============================================================
# Quality Gate — Local Pre-Push Analysis (11 steps)
# Catches the same issues as Copilot/Cursor bots and SonarQube
# ============================================================
...
echo -e "\n${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BOLD}${BLUE}  Quality Gate — Local Pre-Push Analysis (${TOTAL_STEPS} steps)${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

# ============================================================
# [1/11] Gitleaks — Secret detection
# ============================================================
echo -e "\n${CYAN}[1/${TOTAL_STEPS}] Gitleaks — Scanning for secrets in the code...${NC}"
if command -v gitleaks &> /dev/null; then
  GITLEAKS_OUTPUT=$(gitleaks detect --source . --no-git --quiet 2>&1 || true)
  GITLEAKS_EXIT=$?
  if [ $GITLEAKS_EXIT -eq 0 ]; then
    step_pass "Gitleaks: no secrets found"
  else
  ...
  step_warn "Gitleaks not installed — skipping"
  echo -e "${YELLOW}  Install: brew install gitleaks${NC}"
fi

# ============================================================
# [2/11] TypeScript — Compilation
# ============================================================
...
# ============================================================
# [3/11] ESLint — Code quality
# ============================================================
echo -e "\n${CYAN}[3/${TOTAL_STEPS}] ESLint — Checking code quality rules...${NC}"
...
if [ "$ESLINT_ERRORS" -gt 0 ]; then
  step_fail "ESLint: ${ESLINT_ERRORS} error(s)"
  ...
  step_pass "ESLint: no issues found"
fi

# ============================================================
# [4/11] Prettier — Formatting
# ============================================================
echo -e "\n${CYAN}[4/${TOTAL_STEPS}] Prettier — Checking code formatting...${NC}"
...
if echo "$PRETTIER_OUTPUT" | grep -q "All matched files use Prettier code style"; then
  step_pass "Prettier: formatting OK"
else
  UNFORMATTED=$(echo "$PRETTIER_OUTPUT" | grep -c "\.ts$" 2>/dev/null || echo "0")
  step_fail "Prettier: ${UNFORMATTED} file(s) with incorrect formatting"
  echo -e "${YELLOW}  Fix: npx prettier --write 'src/**/*.ts'${NC}"
fi

# ============================================================
# [5/11] npm audit — Dependency vulnerabilities
# ============================================================
echo -e "\n${CYAN}[5/${TOTAL_STEPS}] npm audit — Checking dependencies...${NC}"
...
if [ "$CRITICAL" -gt 0 ]; then
  step_fail "npm audit: ${CRITICAL} CRITICAL vulnerability(ies)"
  echo -e "${YELLOW}  Fix: npm audit fix${NC}"
elif [ "$HIGH" -gt 0 ]; then
  step_warn "npm audit: ${HIGH} HIGH vulnerability(ies)"
else
  step_pass "npm audit: no critical vulnerabilities"
fi

# ============================================================
# [6/11] Knip — Dead code
# ============================================================
echo -e "\n${CYAN}[6/${TOTAL_STEPS}] Knip — Checking for dead code...${NC}"
if npx knip --version &> /dev/null 2>&1; then
  ...
  if [ "$TOTAL_DEAD" -gt 0 ]; then
    ...
    step_pass "Knip: no dead code detected"
  fi
else
  step_warn "Knip not installed — skipping"
  echo -e "${YELLOW}  Install: npm install -D knip${NC}"
fi

# ============================================================
# [7/11] Jest — Tests + Coverage
# ============================================================
echo -e "\n${CYAN}[7/${TOTAL_STEPS}] Jest — Running tests with coverage...${NC}"
...
# ============================================================
# [8/11] SonarQube (optional — only runs if container is UP)
# ============================================================
echo -e "\n${CYAN}[8/${TOTAL_STEPS}] SonarQube — Static analysis...${NC}"
...
if curl -s "${SONAR_HOST_URL}/api/system/status" 2>/dev/null | grep -q '"status":"UP"'; then
  ...
  step_warn "SonarQube is not running — skipping"
  echo -e "${YELLOW}  Start it: docker compose up -d${NC}"
fi

# ============================================================
# [9/11] API Lint — OpenAPI Contract Validation (Spectral)
# ============================================================
    ...
    if [ -n "${OPENAPI_FILE}" ] && [ -f "${OPENAPI_FILE}" ]; then
      ...
      if [ "${SPECTRAL_ERRORS:-0}" -gt 0 ]; then
        if [ "${API_LINT_SEVERITY:-warn}" = "error" ]; then
          step_fail "API Lint: ${SPECTRAL_ERRORS} error(s), ${SPECTRAL_WARNINGS} warning(s)"
        else
          step_warn "API Lint: ${SPECTRAL_ERRORS} error(s), ${SPECTRAL_WARNINGS} warning(s)"
        fi
        echo "${SPECTRAL_OUTPUT}" | head -20
      elif [ "${SPECTRAL_WARNINGS:-0}" -gt 0 ]; then
        step_warn "API Lint: ${SPECTRAL_WARNINGS} warning(s)"
      else
        step_pass "API Lint: OpenAPI contract is valid"
      fi
    else
      step_warn "API Lint: no OpenAPI file found — skipping"
    fi
  else
    step_warn "Spectral not installed — skipping"
    echo -e "${YELLOW}  Install: npm install -g @stoplight/spectral-cli${NC}"
  fi
else
  step_warn "API Lint disabled (ENABLE_API_LINT=false). Enable: ENABLE_API_LINT=true ./quality-gate.sh"
fi

# ============================================================
# [10/11] Infra Scan — Infrastructure Security (Trivy)
# ============================================================
...
" 2>/dev/null || echo "0")
    ...
    if [ "${INFRA_FINDINGS}" -gt 0 ]; then
      step_warn "Infra Scan: ${INFRA_FINDINGS} security finding(s)"
    else
      step_pass "Infra Scan: infrastructure is secure"
    fi
  else
    step_warn "Trivy not installed — skipping"
    echo -e "${YELLOW}  Install: https://aquasecurity.github.io/trivy/latest/getting-started/installation/${NC}"
  fi
else
  step_warn "Infra Scan disabled (ENABLE_INFRA_SCAN=false). Enable: ENABLE_INFRA_SCAN=true ./quality-gate.sh"
fi

# ============================================================
# [11/11] Stryker — Mutation Testing (optional, slow)
# ============================================================
  ...
  if npx stryker --version &> /dev/null 2>&1; then
    ...
    if [ -n "$MUTATION_SCORE" ]; then
      ...
      if [ "$SCORE_INT" -ge 60 ]; then
        ...
        step_warn "Stryker: mutation score ${MUTATION_SCORE}% (below 60%)"
      fi
    else
      step_warn "Stryker: could not extract mutation score"
    fi
  else
    step_warn "Stryker not installed — skipping"
    echo -e "${YELLOW}  Install: npm install -D @stryker-mutator/core @stryker-mutator/jest-runner @stryker-mutator/typescript-checker${NC}"
  fi
else
  step_warn "Stryker disabled (slow). Enable: RUN_STRYKER=true ./quality-gate.sh"
fi
...
if [ $ERRORS -gt 0 ]; then
  ...
  echo -e "${RED}  Fix all errors before pushing.${NC}"
  echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
  exit 1
elif [ $WARNINGS -gt 0 ]; then
  echo -e "${YELLOW}${BOLD}  QUALITY GATE PASSED with ${WARNINGS} warning(s)${NC}"
  echo -e "${YELLOW}  Consider fixing the warnings.${NC}"
  echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
  exit 0
else
